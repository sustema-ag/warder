...
- [Report a bug](https://github.com/sustema-ag/warder/issues/new?labels=bug&template=BUG.md)
- [Propose a new feature](https://github.com/sustema-ag/warder/issues/new?labels=enhancement&template=FEATURE.md)
- [Send a pull request](https://github.com/sustema-ag/warder/pulls)
...
same [MIT](https://github.com/sustema-ag/warder/blob/main/LICENSE)
...
Please do not report a bug for a version of `warder` that is no longer
supported. Please do not report a bug if you are using a version of Erlang or
Elixir that is not supported by the version of `warder` you are using.
...
We are trying to keep backwards compatibility breaks in `warder` to a minimum.
...
## Using `warder` from a Git checkout

The following commands can be used to perform the initial checkout of
`warder`:

```bash
$ git clone git@github.com:sustema-ag/warder.git

$ cd warder
```

Install `warder`'s dependencies using [mix](https://hexdocs.pm/mix/Mix.html):

```bash
$ mix deps.get
```

## Running `warder`'s test suite

After following the steps shown above, `warder`'s test suite is run like
this:
...
## Generating `warder` Documentation
